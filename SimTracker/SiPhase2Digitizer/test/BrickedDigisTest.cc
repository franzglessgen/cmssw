...
// -*- C++ -*-
//
// Package:    BrickedDigisTest
// Class:      BrickedDigisTest
//
/**\class BrickedDigisTest BrickedDigisTest.cc 
...
#include <cmath>
class BrickedDigisTest : public DQMEDAnalyzer {
public:
  explicit BrickedDigisTest(const edm::ParameterSet&);
  ~BrickedDigisTest() override;
...
// constructors
//
BrickedDigisTest::BrickedDigisTest(const edm::ParameterSet& iConfig)
    : config_(iConfig),
  ...
  edm::LogInfo("BrickedDigisTest") << ">>> Construct BrickedDigisTest ";
}

//
// destructor
//
BrickedDigisTest::~BrickedDigisTest() {
  // do anything here that needs to be done at desctruction time
  // (e.g. close files, deallocate resources etc.)
  edm::LogInfo("BrickedDigisTest") << ">>> Destroy BrickedDigisTest ";
}
//
// -- Analyze
//
void BrickedDigisTest::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  ...
  if (theTkDigiGeomWatcher.check(iSetup)) {
    ...
    for (DSViter = digis->begin(); DSViter != digis->end(); DSViter++) {
      ...
      edm::LogInfo("BrickedDigisTest") << " Det Id = " << rawid;
	  ...
	  int row = di->row();     // row
          ...
          nDigi++;
          edm::LogInfo("BrickedDigisTest") << "  column " << col << " row " << row << std::endl;
          ...
          edm::LogInfo("BrickedDigisTest") << " row " << row << " col " << col << " row_last " << row_last << " col_last "
...
// -- Book Histograms
//
void BrickedDigisTest::bookHistograms(DQMStore::IBooker& ibooker, edm::Run const& iRun, edm::EventSetup const& iSetup) {
  ...
  for (const auto& itype : types) {
    ...
    edm::LogInfo("BrickedDigisTest") << " Booking Histograms in : " << folder_name.str();
...
int BrickedDigisTest::matchedSimTrackIndex(edm::Handle<edm::DetSetVector<PixelDigiSimLink> >& linkHandle,
                                    edm::Handle<edm::SimTrackContainer>& simTkHandle,
                                    DetId detId,
                                    unsigned int& channel) {
  int simTrkIndx = -1;
...
void BrickedDigisTest::fillClusterWidth(DigiMEs& mes, float dphi, float width) {
  for (unsigned int i = 0; i < phiValues.size(); i++) {
...
DEFINE_FWK_MODULE(BrickedDigisTest);
