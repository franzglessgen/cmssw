...
BrickedTBValidation::BrickedTBValidation(const edm::ParameterSet& iConfig)
    : config_(iConfig),
  ...
  LogDebug("BrickedTBValidation") << ">>> Construct BrickedTBValidation ";
  ...
  if (prov_ref_m.size() != 0) {
    ...
    use_this_track_ = std::bind(&BrickedTBValidation::_check_input_angles_, this, std::placeholders::_1);
...
// destructor
//
BrickedTBValidation::~BrickedTBValidation() {
  LogDebug("BrickedTBValidation") << ">>> Destroy BrickedTBValidation ";
}
...
// -- Analyze
//
void BrickedTBValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
...
// -- Book Histograms
//
void BrickedTBValidation::bookHistograms(DQMStore::IBooker& ibooker,
                                             edm::Run const& iRun,
  ...
  for (auto const& dunit : tkGeom->detUnits()) {
    ...
    if (vME_position_cell_.find(me_unit) == vME_position_cell_.end()) {
      ...
      std::cout << "BrickedTBValidation"
                << "Booking Histograms in: " << folder_name << " ME UNIT:" << me_unit << std::endl;
      edm::LogInfo("BrickedTBValidation") << "Booking Histograms in: " << folder_name << std::endl;
    }
  }
}

bool BrickedTBValidation::isPixelSystem_(const GeomDetUnit* dunit) const {
  const auto& dtype = dunit->type();
  if (!dtype.isInnerTracker() || !dtype.isTrackerPixel()) {
    return false;
  }
...
int BrickedTBValidation::meUnit_(bool isBarrel, int layer, int side) const {
  // [isBarrel][layer][side]
  // [X][XXX][XX]
  return (static_cast<int>(isBarrel) << 6) | (layer << 3) | side;
}

BrickedTBValidation::MonitorElement* BrickedTBValidation::setupH1D_(DQMStore::IBooker& ibooker,
...
BrickedTBValidation::MonitorElement* BrickedTBValidation::setupH2D_(DQMStore::IBooker& ibooker,
                                                                            const std::string& histoname,
                                                                            const std::string& title) {
  // Config need to have exactly the same histo name
  edm::ParameterSet params = config_.getParameter<edm::ParameterSet>(histoname);
...
BrickedTBValidation::MonitorElement* BrickedTBValidation::setupProf2D_(
...
const PixelDigi& BrickedTBValidation::get_digi_from_channel_(
...
const SimTrack* BrickedTBValidation::get_simtrack_from_id_(unsigned int idx, const edm::SimTrackContainer* stc) {
  ...
  //edm::LogWarning("BrickedTBValidation::get_simtrack_from_id_")
  edm::LogInfo("BrickedTBValidation::get_simtrack_from_id_") << "Not found any SimTrack with trackId: " << idx;
  return nullptr;
}

const std::vector<const PSimHit*> BrickedTBValidation::get_simhits_from_trackid_(
...
const std::pair<double, double> BrickedTBValidation::pixel_cell_transformation_(
...
//bool BrickedTBValidation::channel_iluminated_by_(const MeasurementPoint & localpos,int channel, double tolerance) const
//{
...
bool BrickedTBValidation::channel_iluminated_by_(const PSimHit& ps,
                                                     int channel,
...
std::set<std::pair<int, int>> BrickedTBValidation::get_illuminated_pixels_(const PSimHit& ps,
                                                                               const PixelGeomDetUnit* tkDetUnit) {
...
bool BrickedTBValidation::_check_input_angles_(const PSimHit* psimhit) {
...
DEFINE_FWK_MODULE(BrickedTBValidation);
